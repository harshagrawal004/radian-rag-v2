...
export async function getMockIntroMessage(patientId: string): Promise<string> {
  ...
  return `Sanjeev's recent data shows rising HbA1c and stable cardiac metrics. Key areas to explore:

🔹 Review glycemic control trends (6-month pattern)
🔹 Assess cardiac medication effectiveness
🔹 Evaluate exercise-BP correlation
🔹 Discuss dietary impact on HbA1c`;
}
